...
load(['D:\matlab2018\bin\work new\自己编写的代码\我自己写的信号识别\第二篇识别\进行分类操作\随机森林1\训练集样本数目测试\优化\Gini.mat']);%dd1
%load Gini;%dd2
...
P_train=[input_ML(temp_weizhen(1:5*j),dd2(1:5*k)');input_ML((250.+temp_baopo(1:5*j)),dd2(1:5*k)')];
T_train=[label(1:(5*j));label(251:(250+5*j))];
P_test=[input_ML(201:250,dd2(1:5*k)');input_ML(451:500,dd2(1:5*k)')];
...
save(['D:\matlab2018\bin\work new\自己编写的代码\我自己写的信号识别\第二篇识别\进行分类操作\随机森林1\训练集样本数目测试\优化\Gini分类结果\2\model',num2str(5*j),'.mat']);
clearvars -except j
end